...
def main(translator=None):
    ...
    for attempt in range(1, total_attempts + 1):
        print(f"\n{Fore.CYAN}{EMOJI['INFO']} 开始第 {attempt}/{total_attempts} 次注册{Style.RESET_ALL}")
        registration = CursorRegistration(translator)
        if registration.start():
            successful_registrations += 1
        print(f"{Fore.CYAN}{EMOJI['INFO']} 已完成 {successful_registrations}/{attempt} 次成功注册{Style.RESET_ALL}")
        
        # 在每次注册之间添加短暂延迟
        if attempt < total_attempts:
            time.sleep(5)

    print(f"\n{Fore.GREEN}{EMOJI['DONE']} 注册完成！成功注册数量: {successful_registrations}/{total_attempts}{Style.RESET_ALL}")
    print(f"\n{Fore.CYAN}{'='*50}{Style.RESET_ALL}")
